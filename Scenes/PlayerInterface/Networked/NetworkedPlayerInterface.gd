...
func _new_hand_manager(opponent : CharacterData, interface : OpponentActionsInterface):
	var opponent_hand_manager = _opponent_hand_manager_scene.instantiate()
...
func add_opponent(opponent:CharacterData):
	var interface : OpponentActionsInterface = super.add_opponent(opponent)
...
@rpc("any_peer") func _remote_opponent_draw_card(player_id, card_key : String):
...
@rpc("any_peer") func _remote_opponent_discard_card(player_id : int, card_key : String):
	var opponent : CharacterData = Network.get_player_character(player_id)
...
@rpc("any_peer") func _remote_opponent_move_card(player_id : int, card_key : String, position_x : float, position_y : float, rotation : float, scale_x : float, scale_y : float, tween_time : float):
	var opponent : CharacterData = Network.get_player_character(player_id)
...
@rpc("any_peer") func _remote_opponent_play_card(player_id : int, card_key : String):
	var opponent : CharacterData = Network.get_player_character(player_id)
...
func _on_play_card_on_opportunity(card:CardData, opportunity: OpportunityData):
	super._on_play_card_on_opportunity(card, opportunity)
